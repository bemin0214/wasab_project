...
| `a` | April marker가 보일 때까지 J1을 좌우로 계속 스캔하고, marker를 찾으면 즉시 정지 |
| `t` | grasp 이후 throw 동작 수행 |
| `w` | 물체를 잡은 상태에서 home 위치로 이동 |
| `q` | 종료 |

노트북 AdminGUI에서도 같은 기능을 사용할 수 있습니다.

| 버튼/명령 | 동작 |
| --- | --- |
| `Find Marker` / `find-marker` | 라즈베리파이/Jetcobot 클라이언트에 marker 찾기 명령 전송 |

### April marker 찾기 설정

marker 찾기 기능은 라즈베리파이/Jetcobot 쪽에서 실행됩니다. 설정은 아래 파일에서 수정합니다.
...
- `dictionary`: 사용할 AprilTag/Aruco dictionary입니다. 기본값은 `DICT_APRILTAG_36h11`입니다.
- `target_ids`: 특정 marker ID만 찾을 때 쉼표로 입력합니다. 비워두면 모든 marker를 허용합니다.
- `pan_joint`: 주변을 볼 때 움직일 관절 번호입니다. 기본값은 J1입니다.
- `pan_range_deg`: 현재 J1 각도를 기준으로 좌우 탐색할 범위입니다.
- `search_step_deg`: 한 번에 움직일 각도 간격입니다.
- `max_duration_sec`: `0.0`이면 marker를 찾을 때까지 계속 탐색합니다. 양수로 설정하면 해당 시간 후 중단합니다.
