...
상태 전이 (State):
  ...
  SUPPRESSING     — Yes 수신, 그리퍼 닫음. SUPPRESS_CHECK_DELAY_SEC 후 결과 판정.
"""
...
# S_MIN: 2026-07-24 50→140로 상향. 피부색은 채도가 낮고(옅음) 불꽃은 채도가 매우 높아서(선명함),
# 채도 기준만 올려도 사람 얼굴/손 오탐이 크게 줄어든다(실기에서 사람 얼굴이 불로 잡히는 문제 확인 후 조정).
V_MIN, S_MIN = 90, 140
H_LO_MAX, H_HI_MIN = 35, 160
...
RESPONSE_TIMEOUT_SEC = 15.0    # 프롬프트 후 무응답 시 No와 동일 처리까지 대기(N값, 미정 — 임시)
SUPPRESS_CHECK_DELAY_SEC = 3.0  # 그리퍼 닫은 후 결과 판정까지 대기
...
class FireResponseFSM:
    ...
    def step(self, confirmed, response, now=None):
        ...
        if self.state == "IDLE":
            ...
            if now - self._t_suppress >= SUPPRESS_CHECK_DELAY_SEC:
                self.state = "IDLE"
                if confirmed:      # 그리퍼 동작 후에도 여전히 불이 감지됨 → 실패
...
def main() -> None:
    ...
    parser.add_argument("--response-file",
                        help="AdminGUI Yes/No 응답을 읽을 임시 파일")
    ...
    if args.source:
        ...
        latest = cv2.VideoCapture(source)
    ...
    print(f"[fire_detect] {source_description} 수신 → {args.fire_topic} "
    ...
    try:
        while rclpy.ok():
            ...
            confirmed = False
            if ok:
                bbox, centroid, buf = detect_fire(frame, buf, still=node.arm_still, debug=args.debug)
                ...
                if bbox is not None:
                    ...
                    node.publish_fire(cx, cy, 1.0)
                    print(
                        f"VISION_TARGET cx={cx:.6f} cy={cy:.6f} conf=1.000000",
                        flush=True,
                    )
                # 미검출 시 /wasab/k3/fire 미발행 — search_node의 face_timeout으로 간헐 미검출 흡수

                response = node.take_response()
                if response_file is not None and response_file.exists():
                    try:
                        file_response = response_file.read_text(
                            encoding="utf-8"
                        ).strip().lower()
                        response_file.unlink(missing_ok=True)
                        if file_response in {"yes", "no"}:
                            response = file_response
                    except OSError:
                        pass
                events, suppress = node.fsm.step(confirmed, response)
                if events:
